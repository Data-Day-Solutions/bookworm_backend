...
try:
    from ragbot_tools.rag_chatbot_function_for_content_map_ollama import run_chatbot as run_chatbot_ollama
except (ImportError, ModuleNotFoundError):
    from api.ragbot_tools.rag_chatbot_function_for_content_map_ollama import run_chatbot as run_chatbot_ollama
...
@chat_bp.route('/chatbot_ollama', methods=['POST'])
def chatbot_ollama():
    ...
    if check_session():
        ...
        data = request.get_json()
        user_prompt = data.get('user_prompt')
        chatbot_response = run_chatbot_ollama(user_prompt)
